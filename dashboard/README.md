...
## Update: Notes and trade detail view
- Added a `หมายเหตุ` column to the trade report table with a compact preview.
- Trade rows can be clicked (or focused and opened with Enter/Space) to view full trade details.
- Added public read-only route `/trade/{id}` with price levels, dates, result, R:R and full notes.
- Admin users get a direct edit button from the detail page.
