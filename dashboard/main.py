...
def trade_detail(request: Request, trade_id: int):
    try:
        ...
        raise HTTPException(status_code=503, detail=f"ไม่สามารถอ่านข้อมูลได้: {exc}") from exc
    if not trade:
        raise HTTPException(status_code=404, detail="ไม่พบแผน")
    plan = normalize_trade(trade)
    is_admin = signed_in(request)
    return templates.TemplateResponse(
        request=request,
        name="trade_detail.html",
        context={
            "request": request,
            "p": plan,
            "is_admin": is_admin,
            "back_url": "/admin#trade-report" if is_admin else "/dashboard#trade-report",
        },
    )
